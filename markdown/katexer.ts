...
import type * as StateBlock from "markdown-it/lib-modules/rules_block/state_block";
import type StateCore from "markdown-it/lib-modules/rules_core/state_core";
import type * as StateInline from "markdown-it/lib-modules/rules_inline/state_inline";
import type * as Token from "markdown-it/lib-modules/token";
...
function inlineLatex(state: StateInline, silent: boolean): boolean {
  const startSeq = "\\(";
  const endSeq = "\\)";
  const startLen = startSeq.length;
  const endLen = endSeq.length;
  const start = state.pos;

  if (state.src.slice(start, start + startLen) !== startSeq) {
    return false;
  }

  let matchStart = start + startLen;
  let matchEnd = state.src.indexOf(endSeq, matchStart);

  if (matchEnd === -1) {
    if (!silent) state.pending += state.src.slice(start, matchStart); // Optional: handle unmatched sequences
    state.pos += startLen; // Move past the initial delimiter if no closing found
    return false; // Close the function as the end sequence isn't found
  }
  ...
  if (!silent) {
    ...
    token.markup = startSeq + endSeq;
  }

  state.pos = matchEnd + endLen;
  return true;
}

function inlineLatex2(state: StateInline, silent: boolean): boolean {
  const startSeq = "\\[";
  const endSeq = "\\]";
  const startLen = startSeq.length;
  const endLen = endSeq.length;
...
export default function (
  ...
  md.inline.ruler.before("escape", "math_latex", inlineLatex);
  md.inline.ruler.before("escape", "math_latex2", inlineLatex2);
